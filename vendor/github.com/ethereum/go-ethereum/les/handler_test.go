...
func testGetBlockHeaders(t *testing.T, protocol int) {
	pm := newTestProtocolManagerMust(t, false, downloader.MaxHashFetch+15, nil, nil, nil, database.NewMemDatabase())
...
func testGetBlockBodies(t *testing.T, protocol int) {
	pm := newTestProtocolManagerMust(t, false, downloader.MaxBlockFetch+15, nil, nil, nil, database.NewMemDatabase())
	bc := pm.blockchain.(*core.BlockChain)
	peer, _ := newTestPeer(t, "peer", protocol, pm, true)
	defer peer.close()
...
func testGetCode(t *testing.T, protocol int) {
	...
	pm := newTestProtocolManagerMust(t, false, 4, testChainGen, nil, nil, database.NewMemDatabase())
	bc := pm.blockchain.(*core.BlockChain)
	peer, _ := newTestPeer(t, "peer", protocol, pm, true)
	defer peer.close()
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	case 1:
		...
		trie, _ := trie.New(root, trie.NewDatabase(database.NewTable(db, light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(database.NewTable(db, light.BloomTrieTablePrefix)))
