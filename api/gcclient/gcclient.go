...
// Package gcchain provides a client for the gcchain RPC API.
package gcclient
...
func (c *Client) GetBlockNumber() *big.Int {
...
func (c *Client) ChainConfig() (*configs.ChainConfig, error) {
	...
	if err != nil {
		log.Error("Cann't get chain config", "err", err)
		return nil, err
	}
...
func (c *Client) TransactionByHash(ctx context.Context, hash common.Hash) (tx *types.Transaction, isPending bool, err error) {
	...
	if err != nil {
		...
		return nil, false, gcchain.NotFound
...
func (c *Client) TransactionInBlock(ctx context.Context, blockHash common.Hash, index uint) (*types.Transaction, error) {
	...
	if err == nil {
		if json == nil {
			...
			return nil, fmt.Errorf("S0erver returned transaction without signature")
		}
	}
	if json.From != nil && json.BlockHash != nil {
		setSenderFromServer(json.tx, *json.From, *json.BlockHash)
...
func (c *Client) SyncProgress(ctx context.Context) (*gcchain.SyncProgress, error) {
	var raw json.RawMessage
...
func (c *Client) SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (gcchain.Subscription, error) {
...
func (c *Client) GetRNodes(ctx context.Context) ([]RNodes, error) {
	var result []RNodes
	err := c.c.CallContext(ctx, &result, "eth_getRNodes")
	return result, err
}

func (c *Client) GetCurrentView(ctx context.Context) (uint64, error) {
	var result uint64
	err := c.c.CallContext(ctx, &result, "eth_getCurrentView")
	return result, err
}

func (c *Client) GetCurrentTerm(ctx context.Context) (uint64, error) {
	var result uint64
	err := c.c.CallContext(ctx, &result, "eth_getCurrentTerm")
	return result, err
}

func (c *Client) GetCommitteeNumber(ctx context.Context) (int, error) {
	var result int
...
func (c *Client) FilterLogs(ctx context.Context, q gcchain.FilterQuery) ([]types.Log, error) {
...
func (c *Client) SubscribeFilterLogs(ctx context.Context, q gcchain.FilterQuery, ch chan<- types.Log) (gcchain.Subscription, error) {
	return c.c.EthSubscribe(ctx, ch, "logs", toFilterArg(q))
}

func toFilterArg(q gcchain.FilterQuery) interface{} {
...
func (c *Client) CallContract(ctx context.Context, msg gcchain.CallMsg, blockNumber *big.Int) ([]byte, error) {
...
func (c *Client) PendingCallContract(ctx context.Context, msg gcchain.CallMsg) ([]byte, error) {
	var hex hexutil.Bytes
...
func (c *Client) EstimateGas(ctx context.Context, msg gcchain.CallMsg) (uint64, error) {
...
func toCallArg(msg gcchain.CallMsg) interface{} {
	arg := map[string]interface{}{
		"from": msg.From,
		"to":   msg.To,
	}
