...
const (
	ClientIdentifier = "gcchain" // Client identifier to advertise over the network
...
var (
	// just for test
	TestChainConfig = &ChainConfig{big.NewInt(DevChainId), &DposConfig{Period: 0, TermLen: 4}}
)

// this contains all the changes we have made to the gcchain protocol.
...
type ChainConfig struct {
	...
	Dpos *DposConfig `json:"dpos,omitempty" toml:"dpos,omitempty"`
}

// DposConfig is the consensus engine configs for proof-of-authority based sealing.
...
func (c *DposConfig) ValidatorsLen() uint64 {
	if c != nil {
...
func (c *DposConfig) Certificate(n uint64) bool {
	if c != nil {
...
func (c *DposConfig) ImpeachCertificate(n uint64) bool {
	if c != nil {
...
func (c *DposConfig) PeriodDuration() time.Duration {
	if c != nil {
...
func (c *DposConfig) BlockDelay() time.Duration {
	if c != nil {
...
// Isgcchain returns if it is gcchainDawn era.
func (c *ChainConfig) Isgcchain() bool {
	if c.ChainID == nil {
		return false
	}
...
func (c *ChainConfig) Rules(num *big.Int) Rules {
	...
	return Rules{ChainID: new(big.Int).Set(chainID), Isgcchain: c.Isgcchain()}
}
