...
import (
	...
	"github.com/gcchains/chain/protocols/gcc/fetcher"
...
// NewProtocolManager returns a new sub protocol manager. The gcchain sub protocol manages peers capable
// with the gcchain network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing gcchain peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping gcchain protocol")
	...
	pm.engine.(*dpos.Dpos).StopMining()
	...
	log.Info("gcchain protocol stopped")
}
...
// addPeer is the callback invoked to manage the life cycle of gcchain peer.
...
func (pm *ProtocolManager) addPeer(p *peer, isMinerOrValidator bool) (bool, error) {
	...
	if err != nil {
		log.Debug("gcchain handshake failed", "err", err)
		return false, err
	}
	...
	log.Debug("gcchain peer connected", "name", p.Name())
...
func (pm *ProtocolManager) handlePeer(p *p2p.Peer, rw p2p.MsgReadWriter, version uint) error {
	var (
		dposEngine         = pm.engine.(*dpos.Dpos)
		isMiner            = dposEngine.IsMiner()
		workAsValidator    = dposEngine.IsValidator()
		dposMode           = dposEngine.Mode()
		dposProtocol       = dposEngine.Protocol()
		isMinerOrValidator = isMiner || workAsValidator
		handleTxs          = !workAsValidator
		handleDposMsgs     = workAsValidator && dposMode == dpos.NormalMode
	)

	if dposMode == dpos.NormalMode && isMinerOrValidator {
		for !dposProtocol.Available() {
			log.Debug("dpos protocol is not available now, sleeping 5 seconds.")
			time.Sleep(5 * time.Second)
		}
		log.Debug("now dpos protocol is available")
	}
	...
	case pm.newPeerCh <- peer:
		...
		if err != nil {
			log.Debug("fail to add peer to gcc protocol manager's peer set", "peer.RemoteAddr", peer.RemoteAddr().String(), "local addr", peer.LocalAddr().String(), "peer.id", peer.IDString(), "err", err)
			return err
		}
		...
		log.Debug("is validator and remote is miner", "is validator", dposEngine.IsValidator(), "remote miner", remoteIsMiner, "id", p.ID().String(), "remote addr", p.RemoteAddr().String(), "local addr", p.LocalAddr().String())

		// validator do not connect to civilian to avoid deny of service attack
		if dposEngine.IsValidator() && !remoteIsMiner {
		...
		// add peer to dpos.handler.dialer.peers, this is for proposers/validators communication
		id, added := common.Address{}.Hex(), false
		if dposMode == dpos.NormalMode && isMinerOrValidator && remoteIsMiner {
			switch id, _, _, err = dposProtocol.AddPeer(int(version), peer.Peer, peer.rw); err {
			case nil:
				added = true
				log.Debug("done of dpos subprotocol handshake with peer", "id", p.ID().String(), "remote addr", p.RemoteAddr().String(), "coinbase", id, "local addr", p.LocalAddr().String())

			default:
				log.Debug("failed to add peer to dpos peer set", "err", err, "coinbase", id, "remote addr", p.RemoteAddr().String(), "local addr", p.LocalAddr().String())
				return err
			}
		}
		...
		// defer to remove the peer
		if added {
			defer dposProtocol.RemovePeer(id)
		}
		...
		for {
			if id, err = pm.handleMsg(peer, id, handleTxs, handleDposMsgs, dposProtocol); err != nil {
				log.Debug("gcchain message handleing failed", "err", err, "remote addr", peer.RemoteAddr().String(), "local addr", peer.LocalAddr().String())
				return err
			}
		}
...
func (pm *ProtocolManager) handleMsg(p *peer, id string, handleTxs bool, handleDposMsgs bool, dposProtocol consensus.Protocol) (string, error) {
	...
	case backend.IsDposMsg(msg) && handleDposMsgs:
		switch id, err = dposProtocol.HandleMsg(id, p.version, p.Peer, p.rw, msg); err {
		case nil:

		default:
			log.Warn("err when handling dpos msg", "err", err)
		}
...
// NodeInfo represents a short summary of the gcchain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network uint64               `json:"network"` // gcchain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
