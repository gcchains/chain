...
import (
	...
	"github.com/gcchains/chain/protocols/gcc/tracers"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.gcc.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.gcc.blockchain.CurrentBlock()
	default:
		from = api.gcc.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.gcc.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.gcc.blockchain.CurrentBlock()
	default:
		to = api.gcc.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabase(api.gcc.ChainDb())

	if number := start.NumberU64(); number > 0 {
		start = api.gcc.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.config)

				// Trace all the transactions contained within
				for i, tx := range task.block.Transactions() {
					msg, _ := tx.AsMessage(signer)
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.gcc.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.gcc.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			privStateDB, _ := state.New(core.GetPrivateStateRoot(api.gcc.chainDb, block.StateRoot()), pubStateDB.Database())
			// TODO: Pass real remote database
			_, _, _, _, err := api.gcc.blockchain.Processor().Process(block, pubStateDB, privStateDB, nil, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.gcc.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.gcc.blockchain.CurrentBlock()
	default:
		block = api.gcc.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.gcc.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.gcc.engine.VerifyHeader(api.gcc.blockchain, block.Header(), true, block.RefHeader()); err != nil {
		return nil, err
	}
	parent := api.gcc.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.gcc.blockchain, nil)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	pubStateDB, err := api.gcc.blockchain.StateAt(block.StateRoot())
	if err == nil {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.gcc.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.gcc.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) computeStatePrivDB(block *types.Block) (*state.StateDB, error) {
	// If we have the state fully available, use that
	privStatedb, err := api.gcc.blockchain.StatePrivAt(block.StateRoot())
	if err == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.gcc.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &gccapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  gccapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.gcc.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.gcc.blockchain, nil)
