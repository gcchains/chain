...
import (
	...
	"github.com/gcchains/chain/contracts/reward"
...
var (
	...
	testBankBalance = new(big.Int).Mul(big.NewInt(1000000), big.NewInt(configs.Gcc))
...
func newBlockchainWithDB(n int, deployContract bool) (syncer.BlockChain, *database.MemDatabase) {
	...
	blocks, _ := core.GenerateChain(config, genesis, dposFakeEngine, db, remoteDB, n, generator)
	...
	// deploy contract
	if deployContract {
		backend := backends.NewDposSimulatedBackendWithExistsBlockchain(db, blockchain, config)
