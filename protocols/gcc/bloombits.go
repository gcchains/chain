...
const (
	// bloomServiceThreads is the number of goroutines used globally by a gcchain
...
func (s *gcchainService) startBloomHandlers() {
			...
			for {
				...
				case request := <-s.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(s.chainDb, (section+1)*configs.BloomBitsBlocks-1)
						if compVector, err := rawdb.ReadBloomBits(s.chainDb, task.Bit, section, head); err == nil {
							if blob, err := bitutil.DecompressBytes(compVector, int(configs.BloomBitsBlocks)/8); err == nil {
								task.Bitsets[i] = blob
							} else {
								task.Error = err
							}
...
// for the gcchain header bloom filters, permitting blazing fast filtering.
...
func (b *BloomIndexer) Process(header *types.Header) {
	b.gen.AddBloom(uint(header.Number.Uint64()-b.section*b.size), header.LogsBloom)
	b.head = header.Hash()
}
