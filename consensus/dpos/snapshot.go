...
import (
	...
	"github.com/gcchains/chain/consensus/dpos/election"
...
// DposSnapshot is the state of the authorization voting at a given point in time.
type DposSnapshot struct {
	...
	config *configs.DposConfig // Consensus engine parameters to fine tune behavior

	lock sync.RWMutex
}

func (s *DposSnapshot) number() uint64 {
	s.lock.RLock()
...
func (s *DposSnapshot) setNumber(number uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *DposSnapshot) setHash(hash common.Hash) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *DposSnapshot) hash() common.Hash {
	s.lock.RLock()
	defer s.lock.RUnlock()
...
func (s *DposSnapshot) candidates() []common.Address {
	s.lock.RLock()
	defer s.lock.RUnlock()
...
func (s *DposSnapshot) setCandidates(candidates []common.Address) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *DposSnapshot) recentProposers() map[uint64][]common.Address {
	s.lock.RLock()
	defer s.lock.RUnlock()
...
func (s *DposSnapshot) recentValidators() map[uint64][]common.Address {
	s.lock.RLock()
	defer s.lock.RUnlock()
...
func (s *DposSnapshot) getRecentProposers(term uint64) []common.Address {
	s.lock.RLock()
...
func (s *DposSnapshot) getRecentValidators(term uint64) []common.Address {
	s.lock.RLock()
	defer s.lock.RUnlock()
...
func (s *DposSnapshot) setRecentValidators(term uint64, validators []common.Address) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *DposSnapshot) setRecentProposers(term uint64, proposers []common.Address) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
// the genesis block.
func newSnapshot(config *configs.DposConfig, number uint64, hash common.Hash, proposers []common.Address,
	validators []common.Address, mode Mode) *DposSnapshot {
	snap := &DposSnapshot{
		Mode:             mode,
		config:           config,
		Number:           number,
...
func loadSnapshot(config *configs.DposConfig, db database.Database, hash common.Hash) (*DposSnapshot, error) {
...
func (s *DposSnapshot) store(db database.Database) error {
	s.lock.Lock()
	defer s.lock.Unlock()
...
// the original one.
func (s *DposSnapshot) apply(headers []*types.Header, timeToUpdateCommitttee bool, candidateService campaign.CandidateService, rptService rpt.RptService) (*DposSnapshot, error) {
	...
	for _, header := range headers {
		...
		if err != nil {
			log.Warn("DposSnapshot apply header error.", "err", err)
			return nil, err
		}
	}
...
func (s *DposSnapshot) applyHeader(header *types.Header, ifUpdateCommittee bool, candidateService campaign.CandidateService, rptService rpt.RptService) error {
	...
	if len(header.Dpos.Validators) != 0 && len(header.Dpos.Validators) == int(s.config.ValidatorsLen()) {
		// TODO: there is a vulnerability about validators in header, check it!
		// for now, i just do not update validators from header.
		// s.setRecentValidators(term+1, header.Dpos.Validators)
...
func (s *DposSnapshot) updateCandidates(candidateService campaign.CandidateService, seed int64) error {
...
func (s *DposSnapshot) updateRpts(rptService rpt.RptService) (rpt.RptList, error) {
...
func (s *DposSnapshot) isAboutToCampaign() bool {
...
func (s *DposSnapshot) updateProposers(rpts rpt.RptList, seed int64, rptService rpt.RptService) {
	// Elect proposers
...
func (s *DposSnapshot) Term() uint64 {
	return s.TermOf(s.number())
}

// TermOf returns the term index of given block number
func (s *DposSnapshot) TermOf(blockNum uint64) uint64 {
	if blockNum == 0 {
...
func (s *DposSnapshot) ValidatorViewOf(validator common.Address, number uint64) (int, error) {
...
func (s *DposSnapshot) IsValidatorOf(validator common.Address, number uint64) bool {
	_, err := s.ValidatorViewOf(validator, number)
	return err == nil
}

// IsProposerOf returns if an address is a proposer in the given block number
func (s *DposSnapshot) IsProposerOf(signer common.Address, number uint64) (bool, error) {
	if number == 0 {
...
func (s *DposSnapshot) FutureValidatorsOf(number uint64) []common.Address {
	return s.getRecentValidators(s.FutureTermOf(number))
}

// FutureProposersOf returns future proposers of given block number
func (s *DposSnapshot) FutureProposersOf(number uint64) []common.Address {
...
func (s *DposSnapshot) FutureProposerViewOf(signer common.Address, number uint64) (int, error) {
...
func (s *DposSnapshot) IsFutureProposerOf(proposer common.Address, number uint64) bool {
	_, err := s.FutureProposerViewOf(proposer, number)
	return err == nil
}

// InturnOf returns if a signer at a given block height is in-turn or not
func (s *DposSnapshot) InturnOf(number uint64, signer common.Address) bool {
...
func (s *DposSnapshot) StartBlockNumberOfTerm(term uint64) uint64 {
