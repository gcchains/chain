...
func Test_newSnapshot(t *testing.T) {
	snap := newSnapshot(&configs.DposConfig{Period: 3, TermLen: 3, ViewLen: 3}, 1, common.Hash{}, getProposerAddress(), getValidatorAddress(), FakeMode)
...
func Test_loadSnapshot(t *testing.T) {
	...
	testConfig := configs.DposConfig{Period: 3, TermLen: 3, ViewLen: 3}
	cache, _ := lru.NewARC(inMemorySnapshots)
	expectedDposSnapshot := new(DposSnapshot)
	expectedDposSnapshot.config = &testConfig
	tests := []struct {
		name    string
		args    args
		want    *DposSnapshot
...
func TestSnapshot_store(t *testing.T) {
	...
	cache, _ := lru.NewARC(inMemorySnapshots)
	config := &configs.DposConfig{Period: 3, TermLen: 3}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := s.store(tt.args.db); (err != nil) != tt.wantErr {
				t.Errorf("DposSnapshot.store(%v) error = %v, wantErr %v", tt.args.db, err, tt.wantErr)
			}
		})
	}
}
...
func TestSnapshot_apply(t *testing.T) {
	...
	expectedResult := new(DposSnapshot)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("DposSnapshot.apply(%v) error = %v, wantErr %v", tt.args.headers, err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("DposSnapshot.apply(%v) = \n%v, want \n%v", tt.args.headers, got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := s.applyHeader(tt.args.header, true, nil, nil); (err != nil) != tt.wantErr {
				t.Errorf("DposSnapshot.applyHeader(%v) error = %v, wantErr %v", tt.args.header, err, tt.wantErr)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := s.updateCandidates(nil, 0); (err != nil) != tt.wantErr {
				t.Errorf("DposSnapshot.updateCandidates(%v) error = %v, wantErr %v", tt.args.header, err, tt.wantErr)
			}
		})
	}
}
...
func TestSnapshot_updateRpts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("DposSnapshot.updateRpts(%v) error = %v, wantErr %v", tt.args.header, err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("DposSnapshot.updateRpts(%v) = %v, want %v", tt.args.header, got, tt.want)
			}
		})
	}
}
...
func TestSnapshot_isSigner(t *testing.T) {
	snap := newSnapshot(&configs.DposConfig{Period: 3, TermLen: 3, ViewLen: 3}, 1, common.Hash{}, getProposerAddress()[1:2], getValidatorAddress(), FakeMode)
...
func createSnapshot() *DposSnapshot {
	proposers := getProposerAddress()
	validators := getValidatorAddress()
	config := &configs.DposConfig{Period: 3, TermLen: 3, ViewLen: 3}
	// cache, _ := lru.NewARC(inmemorySnapshots)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := s.candidates(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("DposSnapshot.candidates() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func Test_loadSnapshot_marshal(t *testing.T) {
	cfg := &configs.DposConfig{Period: 3, ViewLen: 3, TermLen: 3}
	...
	recentP[1] = proposers
	snapshot := &DposSnapshot{config: cfg, RecentProposers: recentP}
...
func TestDposSnapshot_updateProposers(t *testing.T) {
