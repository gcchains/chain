...
func (s *RemoteSigner) SetPeer(cpcVersion int, dposVersion int, role Role, p *p2p.Peer, rw p2p.MsgReadWriter) {
	s.lock.Lock()
	defer s.lock.Unlock()

	s.cpcVersion, s.dposVersion, s.role, s.Peer, s.rw = cpcVersion, dposVersion, role, p, rw
}

// Handshake tries to handshake with remote validator
func Handshake(p *p2p.Peer, rw p2p.MsgReadWriter, mac string, sig []byte, term uint64, futureTerm uint64) (address common.Address, dposVersion int, err error) {
	...
	return address, dposVersion, nil
}

// ReadSignerStatus reads status of remote validator
func ReadSignerStatus(msg p2p.Msg, signerStatusData *SignerStatusData) (address common.Address, dposVersion int, err error) {
	...
	if valid {
		dposVersion = int(signerStatusData.ProtocolVersion)
		return
	}
