...
func NewLBFT2(faulty uint64, dpos DposService, handleImpeachBlock HandleGeneratedImpeachBlock, handleFailbackImpeachBlock HandleGeneratedImpeachBlock, db database.Database) *LBFT2 {
	...
	lbft := &LBFT2{
		...
		faulty: faulty,
		number: dpos.GetCurrentBlock().NumberU64() + 1,
		dpos:   dpos,
...
func (p *LBFT2) realFSM(input *BlockOrHeader, msgCode MsgCode, state consensus.State) ([]*BlockOrHeader, Action, MsgCode, consensus.State, error) {
	...
	if p.dpos.HasBlockInChain(hash, number) {
...
func (p *LBFT2) IdleHandler(input *BlockOrHeader, msgCode MsgCode, state consensus.State) ([]*BlockOrHeader, Action, MsgCode, consensus.State, error) {
	...
	case PreprepareMsgCode:
		...
		return p.handlePreprepareMsg(input, state, func(block *types.Block) error {
			...
			return p.dpos.ValidateBlock(block, false, true)
		})
...
func (p *LBFT2) handlePreprepareMsg(input *BlockOrHeader, state consensus.State, blockVerifyFn VerifyBlockFn) ([]*BlockOrHeader, Action, MsgCode, consensus.State, error) {
	...
	parent := p.dpos.GetBlockFromChain(block.ParentHash(), block.NumberU64()-1)
	// if received a preprepare msg, and current time is after parent.timestamp+period+blockDelay, drop it!
	if parent != nil && time.Now().After(parent.Timestamp().Add(p.dpos.Period()).Add(p.dpos.BlockDelay())) {
...
func (p *LBFT2) composePrepareMsg(block *types.Block) (*types.Header, error) {
	...
	switch err := p.dpos.SignHeader(header, consensus.Prepare); err {
	case nil:
...
func (p *LBFT2) composeImpeachPrepareMsg(block *types.Block) (*types.Header, error) {
	...
	switch err := p.dpos.SignHeader(header, consensus.ImpeachPrepare); err {
	case nil:
...
func (p *LBFT2) composeCommitMsg(h *types.Header) (*types.Header, error) {
	...
	switch err := p.dpos.SignHeader(header, consensus.Commit); err {
	case nil:
...
func (p *LBFT2) composeImpeachCommitMsg(h *types.Header) (*types.Header, error) {
	...
	switch err := p.dpos.SignHeader(header, consensus.ImpeachCommit); err {
	case nil:
...
func (p *LBFT2) handleValidateMsg(input *BlockOrHeader, state consensus.State) ([]*BlockOrHeader, Action, MsgCode, consensus.State, error) {
	...
	err := p.dpos.InsertChain(block)
	if err == nil {
		go p.dpos.BroadcastBlock(block, true)
...
func (p *LBFT2) refreshSignatures(header *types.Header, state consensus.State) error {
	...
	signers, signatures, err := p.dpos.ECRecoverSigs(header, state)
	if err != nil {
	...
	// get validators from dpos service
	validators, err := p.dpos.ValidatorsOf(header.Number.Uint64())
	if err != nil {
...
func (p *LBFT2) unknownAncestorBlockHandler(block *types.Block) {
	...
	if p.dpos.TermOf(number) > p.dpos.TermOf(p.number) {
		...
		go p.dpos.Synchronize()
	}

	// recover proposer's address
	proposer, err := p.dpos.ECRecoverProposer(block.Header())
	if err != nil {
		return
	}

	// verify if a legit proposer
	term := p.dpos.TermOf(number)
	isP, err := p.dpos.VerifyProposerOf(proposer, term)
	if err != nil {
		return
	}
...
func (p *LBFT2) tryToImpeach() {
	...
	if impeachBlock, err := p.dpos.CreateImpeachBlock(); impeachBlock != nil && impeachBlock.NumberU64() != failbackNumber && err == nil {
		...
		time.AfterFunc(
			...
			func() {
				currentBlock := p.dpos.GetCurrentBlock()
...
func (p *LBFT2) tryToImpeachFailback() {
	...
	if firstImpeach, secondImpeach, err := p.dpos.CreateFailbackImpeachBlocks(); firstImpeach != nil && secondImpeach != nil && err == nil {
