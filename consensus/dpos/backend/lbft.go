...
func (l *LBFT) Handle(msg p2p.Msg, p *RemoteSigner) error {
	...
	case msg.Code == PreprepareBlockMsg:
		...
		if vh.dpos.HasBlockInChain(hash, number) {
			return nil
		}
		...
		go vh.reBroadcast(NewBOHFromBlock(block), PreprepareMsgCode)
		...
		switch err := vh.dpos.ValidateBlock(block, false, false); err {
		case nil:
			...
			// sign the block
			switch e := vh.dpos.SignHeader(header, consensus.Commit); e {
			case nil:
				// succeed to sign
	...
	case msg.Code == CommitHeaderMsg:
		...
		log.Debug("received signed commit header", "number", number, "hash", hash.Hex(), "signatures", header.Dpos.SigsFormatText())
		...
		go vh.reBroadcast(NewBOHFromHeader(header), CommitMsgCode)

		// verify the prepare header
		// if correct, insert the block into chain, then broadcast it
		switch err := vh.dpos.VerifyHeaderWithState(header, consensus.Commit); err {
		...
		case nil:
			...
			go vh.dpos.BroadcastBlock(blk, true)  // broadcast the block to some random peers (root of peer set size)
			go vh.dpos.BroadcastBlock(blk, false) // broadcast the block header to other peers
		...
		case consensus.ErrNotEnoughSigs:
			...
			case nil:
				...
				// switch err := vh.dpos.VerifyHeaderWithState(header, consensus.Prepared); err {
				// case nil:

				// 	// if the block body of the header is not found and it's not in local chain
				// 	// broadcast the header again
				// 	block, err := vh.knownBlocks.GetBlock(header.Number.Uint64())
				// 	if block == nil && !vh.dpos.HasBlockInChain(header.Hash(), header.Number.Uint64()) {
