...
// SetDposService sets dpos service to dialer
func (d *Dialer) SetDposService(dpos DposService) {
	d.dpos = dpos
}

// AddPeer adds a peer to local dpos peer set:
...
func (d *Dialer) isCurrentOrFutureProposer(address common.Address, term uint64, futureTerm uint64) bool {
	...
	for t := term; t <= futureTerm; t++ {
		isP, _ := d.dpos.VerifyProposerOf(address, t)
...
func (d *Dialer) isCurrentOrFutureValidator(address common.Address, term uint64, futureTerm uint64) bool {
	isValidator := false
	for t := term; t <= futureTerm; t++ {
		isV, _ := d.dpos.VerifyValidatorOf(address, t)
...
func (d *Dialer) addPeer(cpcVersion int, p *p2p.Peer, rw p2p.MsgReadWriter, mac string, sig []byte, term uint64, futureTerm uint64) (string, bool, bool, error) {
	...
	coinbase, dposVersion, err := Handshake(p, rw, mac, sig, term, futureTerm)
	...
	if (!isProposer && !isValidator) || err != nil {
		log.Debug("failed to handshake in dpos", "err", err, "isProposer", isProposer, "isValidator", isValidator)
		return "", isProposer, isValidator, err
	}

	// if the remote peer is a proposer, add it to local peer set
	if isProposer {
		remoteProposer, err := d.addRemoteProposer(cpcVersion, dposVersion, p, rw, coinbase)
	...
	if isValidator {
		remoteValidator, err := d.addRemoteValidator(cpcVersion, dposVersion, p, rw, coinbase)
...
func (d *Dialer) addRemoteProposer(cpcVersion int, dposVersion int, p *p2p.Peer, rw p2p.MsgReadWriter, address common.Address) (*RemoteProposer, error) {
	...
	// add proposer
	remoteProposer.SetPeer(cpcVersion, dposVersion, Proposer, p, rw)
...
func (d *Dialer) addRemoteValidator(cpcVersion int, dposVersion int, p *p2p.Peer, rw p2p.MsgReadWriter, address common.Address) (*RemoteValidator, error) {
	...
	// add validator
	remoteValidator.SetPeer(cpcVersion, dposVersion, Validator, p, rw)
...
func (d *Dialer) AllUselessProposers() map[common.Address]*RemoteProposer {
	...
	for _, addr := range addrs {
		...
		if currentBlock := d.dpos.GetCurrentBlock(); currentBlock != nil {
			var (
				currentNumber = currentBlock.NumberU64()
				currentTerm   = d.dpos.TermOf(currentNumber)
				futureTerm    = d.dpos.FutureTermOf(currentNumber)
			)
...
func (d *Dialer) ProposersOfTerm(term uint64) map[common.Address]*RemoteProposer {
	...
	for _, addr := range addrs {
		...
		proposer, ok := d.recentProposers.Get(addr)
		isProposerOfTerm, err := d.dpos.VerifyProposerOf(address, term)
...
func (d *Dialer) ValidatorsOfTerm(term uint64) map[common.Address]*RemoteValidator {
	...
	for _, addr := range addrs {
		...
		isValidatorOfTerm, err := d.dpos.VerifyValidatorOf(address, term)
...
func (d *Dialer) KeepConnection() {
	futureTimer := time.NewTicker(d.dpos.Period() / 2)
	defer futureTimer.Stop()
	for {
		select {
		case <-futureTimer.C:
			if current := d.dpos.GetCurrentBlock(); current != nil {
				var (
					currentNum  = current.NumberU64()
					currentTerm = d.dpos.TermOf(currentNum)
					futureTerm  = d.dpos.FutureTermOf(currentNum)
					address     = d.dpos.Coinbase()
				)
...
func (d *Dialer) EnoughValidatorsOfTerm(term uint64) (validators map[common.Address]*RemoteValidator, enough bool) {
	validators = d.ValidatorsOfTerm(term)
	enough = len(validators) >= int(d.dpos.Faulty()*2)
	return
}
...
func (d *Dialer) EnoughImpeachValidatorsOfTerm(term uint64) (validators map[common.Address]*RemoteValidator, enough bool) {
	validators = d.ValidatorsOfTerm(term)
	enough = len(validators) >= int(d.dpos.Faulty())
	return
}
