...
func (vh *Handler) handleSignerConnectionMsg(cpcVersion int, p *p2p.Peer, rw p2p.MsgReadWriter, msg p2p.Msg) (string, error) {
	...
	case NewSignerMsg:
		...
		address, dposVersion, err := ReadSignerStatus(msg, &signerStatus)
		if err != nil {
			return common.Address{}.Hex(), err
		}

		blk := vh.dpos.GetCurrentBlock()
		if blk == nil {
			return "", errNilBlock
		}
		...
		var (
			...
			term          = vh.dpos.TermOf(currectNumber)
			futureTerm    = vh.dpos.FutureTermOf(currectNumber)
		)

		// if current or future proposer, add to local peer set
		if vh.dialer.isCurrentOrFutureProposer(address, term, futureTerm) {
			vh.dialer.addRemoteProposer(cpcVersion, dposVersion, p, rw, address)
		...
		if vh.dialer.isCurrentOrFutureValidator(address, term, futureTerm) {
			vh.dialer.addRemoteValidator(cpcVersion, dposVersion, p, rw, address)
...
func (vh *Handler) handleLBFT2Msg(msg p2p.Msg, p *RemoteSigner) error {
	...
	currentBlock := vh.dpos.GetCurrentBlock()
	if currentBlock == nil {
	...
	case ImpeachValidateMsgCode:
		...
		correctProposer, err := vh.dpos.ProposerOf(input.Number())
		if err != nil {
...
func (vh *Handler) ReceiveFailbackImpeachBlock(block *types.Block) error {
	...
	term := vh.dpos.TermOf(block.NumberU64())
	for {
...
func newMsgID(number uint64, hash common.Hash, msgCode MsgCode, signatures []types.DposSignature) msgID {
...
func (br *broadcastRecord) markAsBroadcasted(number uint64, hash common.Hash, msgCode MsgCode, signatures []types.DposSignature) {
	msgID := newMsgID(number, hash, msgCode, signatures)
	br.record.Add(msgID, true)
}

func (br *broadcastRecord) ifBroadcasted(number uint64, hash common.Hash, msgCode MsgCode, signatures []types.DposSignature) bool {
	msgID := newMsgID(number, hash, msgCode, signatures)
...
func (vh *Handler) reBroadcast(input *BlockOrHeader, msgCode MsgCode) {
	var signatures []types.DposSignature

	if input.IsBlock() {
		signatures = input.block.Dpos().Sigs
	} else if input.IsHeader() {
		signatures = input.header.Dpos.Sigs
	} else {
		return
	}
