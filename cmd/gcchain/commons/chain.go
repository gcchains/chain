...
func OpenChain(ctx *cli.Context, n *node.Node, cfg *gcc.Config) (chain *core.BlockChain, chainDb database.Database) {
	var err error
...
func newBlockChain(config *configs.ChainConfig, chainDb database.Database, trieCache int, n *node.Node, chain *core.BlockChain, err error) *core.BlockChain {
	...
	cacheCfg := &core.CacheConfig{
		...
		TrieNodeLimit: gcc.DefaultConfig.TrieCache,
		TrieTimeLimit: gcc.DefaultConfig.TrieTimeout,
	}
...
// for gcchain and returns half of the allowance to assign to the database.
