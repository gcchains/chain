...
def test_case_1():
    ...
    cf.gcc.defaultAccount = cf.gcc.accounts[0]
    cf.personal.unlockAccount(cf.gcc.accounts[0], "password")
    config = compile_file()
    ...
    network_ins = cf.gcc.contract(abi=config["abi"], address=address)
    print("============read config===================")
    ...
    tx_hash = network_ins.functions.updateHost("google.com").transact({"gas": 7777777})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
    print("set host result: ", tx_receipt["status"])
    tx_hash = network_ins.functions.updateCount(10).transact({"gas": 7777777})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
    print("set count result: ", tx_receipt["status"])
    tx_hash = network_ins.functions.updateTimeout(300).transact({"gas": 7777777})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
    print("set timeout result: ", tx_receipt["status"])
    tx_hash = network_ins.functions.updateGap(10).transact({"gas": 7777777})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
    print("set gap result: ", tx_receipt["status"])
    tx_hash = network_ins.functions.updateOpen(False).transact({"gas": 7777777})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
