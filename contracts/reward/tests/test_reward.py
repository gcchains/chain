...
def deploy_contract(config, cf, owner):
    ...
    reward = cf.gcc.contract(abi=config["abi"], address=contract_address)
...
def generate_nodes(num, cf, owner, value):
    ...
    for i in range(num):
        ...
        enodes.append(enode)
        cf.gcc.sendTransaction({"from": owner, "to": enode, "value": cf.toWei(value, "ether")})
    print("wait for tx confirmation...")
    time.sleep(21)
    for enode in enodes:
        balance = cf.fromWei(cf.gcc.getBalance(enode), "ether")
        print("address: ", enode)
...
def fund_bonus_pool(cf, owner, contract_address):
    tx_hash = cf.gcc.sendTransaction({"from": owner, "to": contract_address, "value": cf.toWei(1250000, "ether")})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
...
def test_case_1():
    ...
    tx_hash = cf.gcc.sendTransaction({"from": owner, "to": contract_address, "value": cf.toWei(20000, "ether")})
    tx_receipt = cf.gcc.waitForTransactionReceipt(tx_hash)
    ...
    for investor in investors:
        ...
        cf.gcc.waitForTransactionReceipt(tx_hash)
...
def build_transaction():
    ...
    config = compile_file()
    contract_address, reward = deploy_contract(config, cf, owner)
    nonce = cf.gcc.getTransactionCount(owner)
