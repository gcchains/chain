...
// PublicgcchainAPI provides an API to access gcchain related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicgcchainAPI struct {
	b Backend
}

// NewPublicgcchainAPI creates a new gcchain protocol API.
func NewPublicgcchainAPI(b Backend) *PublicgcchainAPI {
	return &PublicgcchainAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicgcchainAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current gcchain protocol version this node supports
func (s *PublicgcchainAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicgcchainAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19gcchain Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19gcchain Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an gcchain ECDSA signature for:
// keccack256("\x19gcchain Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19gcchain Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid gcchain signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the gcchain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new gcchain blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

//  GetCurrentView return current RNodes
func (s *PublicBlockChainAPI) GetRNodes() []gcclient.RNodes {
	var rnodes []common.Address
	var committeAddress []common.Address
	var bn uint64

	var RNodes []gcclient.RNodes
...
func getRole(isCommittee bool) int {
	if isCommittee {
		return gcclient.Committee
	}
	return gcclient.Candidate
}
...
func (s *PublicBlockChainAPI) GetBlockGenerationInfo() gcclient.BlockGenerationInfo {
	...
	blockGenerationInfo := gcclient.BlockGenerationInfo{
		ProposerIndex: pi,
		Term:          t,
		View:          v,
...
func submitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if tx.To() == nil {
		...
		log.Info("Submitted transaction", "fullhash", tx.Hash().Hex(), "recipient", tx.To().Hex())
	}
	return tx.Hash(), nil
}
...
// PublicDebugAPI is the collection of gcchain APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of gcchain APIs exposed over the private
// debugging endpoint.
...
// Version returns the current gcchain protocol version.
