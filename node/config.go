...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gcchain is "gcchain". If no
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-gcchain-keystore")
		ephemeral = keydir
	}
...
var DefaultConfig = Config{
	...
	HTTPModules:      []string{"net", "web3", "eth", "gcc"},
...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "gcchain")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "gcchain")
		} else {
			return filepath.Join(home, ".gcchain")
		}
	}
