...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := ReadHeaderRLP(db, header.Hash(), header.Number.Uint64()); entry == nil {
		...
		tmpHeader := []interface{}{
			...
			headerFromRLP.Time,
			headerFromRLP.Dpos.Proposers,
			headerFromRLP.Dpos.Validators,
			headerFromRLP.Extra,
			common.Hash{},
			types.BlockNonce{},
		}
...
func TestBodyStorage(t *testing.T) {
	...
	body := &types.Body{Transactions: []*types.Transaction{types.NewTransaction(0, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil)}}
