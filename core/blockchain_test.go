...
func fakeDpos(db database.Database) *dpos.Dpos {
	return dpos.NewFaker(configs.ChainConfigInfo().Dpos, db)
}
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, comparator func(td1, td2 *big.Int)) {
	...
	blockchain2, err := newCanonical(fakeDpos(db), i, db)
	...
	blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, fakeDpos(db), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	db := database.NewMemDatabase()
	blockchain, err := newCanonical(fakeDpos(db), 0, db)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, fakeDpos(db), blockchain.db, 0)
...
func TestExtendCanonicalBlocks(t *testing.T) {
	...
	db := database.NewMemDatabase()
	processor, err := newCanonical(fakeDpos(db), length, db)
...
func TestBrokenBlockChain(t *testing.T) {
	...
	db := database.NewMemDatabase()
	blockchain, err := newCanonical(fakeDpos(db), 10, db)
	...
	chain := makeBlockChain(blockchain.CurrentBlock(), 5, fakeDpos(db), db, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64) {
	...
	easyBlocks, _ := GenerateChain(configs.TestChainConfig, blockchain.CurrentBlock(), fakeDpos(db), db, remoteDB, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(configs.TestChainConfig, blockchain.CurrentBlock(), fakeDpos(db), db, remoteDB, len(second), func(i int, b *BlockGen) {
...
func TestBlocksInsertNonceError(t *testing.T) {
	...
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		blocks := makeBlockChain(blockchain.CurrentBlock(), i, fakeDpos(db), db, 0)

		failAt = rand.Int() % len(blocks)
		failNum = blocks[failAt].NumberU64()

		config := configs.ChainConfigInfo().Dpos
		d := dpos.NewFakeFailer(config, db, failNum)
...
func TestChainTxReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, fakeDpos(db), db, remoteDB, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, fakeDpos(db), vm.Config{}, remoteDB, nil)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, fakeDpos(db), db, remoteDB, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	var (
		key1, _  = crypto.HexToECDSA("171c71a67e1177ad4e901695e114b9ee17ae16c6668d113eac2f96dbcda3f291")
		addr1    = crypto.PubkeyToAddress(key1.PublicKey)
		db       = database.NewMemDatabase()
		remoteDB = database.NewIpfsDbWithAdapter(database.NewFakeIpfsAdapter())
		// this code generates a log
		code    = common.Hex2Bytes("60606040525b7f24ec1d3ff24c2f6ff210738839dbc339cd41a5294d85c79361016243157aae7b60405180905060405180910310a15b600a8060416000396010f360606040526008565b00")
	...
	chain, _ := GenerateChain(configs.TestChainConfig, genesis, fakeDpos(db), db, remoteDB, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChain(configs.TestChainConfig, genesis, fakeDpos(db), db, remoteDB, 3, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(chain); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func TestReorgSideEvent(t *testing.T) {
	...
	var (
		db       = database.NewMemDatabase()
		remoteDB = database.NewIpfsDbWithAdapter(database.NewFakeIpfsAdapter())
		key1, _  = crypto.HexToECDSA("b71c71a67e1177ad4e901691e1b4b9ee17ae16c6668d313e1c2f96dbcda3f291")
		addr1    = crypto.PubkeyToAddress(key1.PublicKey)
		gspec    = &Genesis{
			Config: configs.TestChainConfig,
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, fakeDpos(db), vm.Config{}, nil, nil)
	defer blockchain.Stop()
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, fakeDpos(db), db, remoteDB, 4, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	...
	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, fakeDpos(db), blockchain.db, blockchain.remoteDB, 10, func(i int, gen *BlockGen) {})
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address, dataFn func(uint64) []byte) {
	var (
		...
		testBankKey, _  = crypto.HexToECDSA("b71c71a67e11771d4e901695e1b4b9ee17ae1616668d313eac2f96dbcda3f291")
