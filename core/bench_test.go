...
func genTxRing(naccounts int) func(int, *BlockGen) {
	from := 0
	return func(i int, gen *BlockGen) {
		gas := CalcGasLimit(gen.PrevBlock(i-1)) - configs.TxGas*2
		for {
			gas -= configs.TxGas
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, dpos.NewFaker(configs.ChainConfigInfo().Dpos, db), db, remoteDB, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, dpos.NewFaker(configs.ChainConfigInfo().Dpos, db), vm.Config{}, remoteDB, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, configs.TestChainConfig, dpos.NewFaker(configs.ChainConfigInfo().Dpos, testdb), vm.Config{}, remoteDB, nil)
		if err != nil {
