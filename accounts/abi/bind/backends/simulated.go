...
type SimulatedBackend struct {
	...
	blockchain *core.BlockChain  // gcchain blockchain to handle the consensus
...
// NewDposSimulatedBackend creates a new binding backend using a simulated blockchain
// for testing purposes.
func NewDposSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
...
// NewDposSimulatedBackendWithExistsBlockchain creates a new binding backend with a exists blockchain
// for test purposes.
func NewDposSimulatedBackendWithExistsBlockchain(db *database.MemDatabase, blockchain *core.BlockChain, config *configs.ChainConfig) *SimulatedBackend {
	backend := &SimulatedBackend{
		database:   db,
		blockchain: blockchain,
		config:     config,
...
func (b *SimulatedBackend) rollback() {
	config := configs.ChainConfigInfo().Dpos
	d := dpos.NewFaker(config, b.database)
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call gcchain.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call gcchain.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call gcchain.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call gcchain.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, uint64, bool, error) {
...
func (b *SimulatedBackend) SendTransaction(ctx context.Context, tx *types.Transaction) error {
	...
	sender, err := types.Sender(types.NewCep1Signer(big.NewInt(configs.MainnetChainId)), tx)
	if err != nil {
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query gcchain.FilterQuery) ([]types.Log, error) {
...
func (b *SimulatedBackend) SubscribeFilterLogs(ctx context.Context, query gcchain.FilterQuery, ch chan<- types.Log) (gcchain.Subscription, error) {
